...
inclui_cabecalho_clean('99 Livros - seção de livros');
...
function exibe_catalogo($a_lista_livros)
{
  ...
  foreach ($a_lista_livros as $line_num => $line)
  {
    ...
    linha( "Título    : ".$titulo);
    linha( "Autor     : ".$autor );
    echo '<img src="./capas/'.$capa.'" style="width:20%; display: block;  margin-top:20px;margin-left: 0;  margin-right: auto">';
    linha( "Categoria : ".$categoria );
    linha( "Publicação: ".$publicacao );
    linha( "Editora   : ".$editora );
    linha( "Páginas   : ".$paginas );
    linha( "ISBN      : ".$isbn );
  }
